 ...
 * Approve submission and create/update restaurant, then return restaurant ID for editing
 */
export async function POST(
  ...
  try {
    ...
    // Create or update restaurant from submission
    ...
    if (existingRestaurant.length > 0) {
      // Update existing restaurant with submission data (keep existing status)
    ...
    await db
      ...
      .set({
        ...
        reviewedBy: 'admin',
        reviewedAt: new Date(),
        notes: 'Approved and opened for editing',
      })
      .where(eq(restaurantSubmissions.id, submissionId));
    ...
    return NextResponse.json({
      ...
      message: isNewRestaurant ? 'Restaurant created and ready for editing' : 'Restaurant updated and ready for editing',
    });
  } catch (error) {
    console.error('Error processing submission edit:', error);
    return NextResponse.json(
